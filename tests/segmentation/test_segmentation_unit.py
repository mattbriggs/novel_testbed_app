...
These are low-level, string-based correctness tests. They do not test
parser integration or pipeline behavior. That is handled elsewhere.
"""
...
def test_segmenter_is_idempotent():
    ...
    The segmenter must never overwrite or damage author-provided structure.
    """
    segmenter = ModuleSegmenter()
    result = segmenter.segment_markdown(ALREADY_SEGMENTED, title="Ignored")

    assert result == ALREADY_SEGMENTED
